defmodule ReverieEngine.Application do
  # See https://hexdocs.pm/elixir/Application.html
  # for more information on OTP Applications
  @moduledoc false

  use Application

  def start(_type, _args) do
    children = [
      # Start the Ecto repository
      ReverieEngine.Repo,
      # Start the Telemetry supervisor
      ReverieEngineWeb.Telemetry,
      # Start the PubSub system
      {Phoenix.PubSub, name: ReverieEngine.PubSub},
      # Start the Endpoint (http/https)
      ReverieEngineWeb.Endpoint,
      # Start a worker by calling: ReverieEngine.Worker.start_link(arg)
      # {ReverieEngine.Worker, arg}
      ReverieMessageToolkit.MessageParser
    ]

    # See https://hexdocs.pm/elixir/Supervisor.html
    # for other strategies and supported options
    opts = [strategy: :one_for_one, name: ReverieEngine.Supervisor]
    Supervisor.start_link(children, opts)
  end

  # Tell Phoenix to update the endpoint configuration
  # whenever the application is updated.
  def config_change(changed, _new, removed) do
    ReverieEngineWeb.Endpoint.config_change(changed, removed)
    :ok
  end
end
